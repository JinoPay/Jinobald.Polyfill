...
public class EnumerableSortingTests
{
    ...
    {
        string[] source = new[] { "apple", "cat", "banana", "ox" };
    ...
    public void OrderBy_빈시퀀스_빈결과반환()
    {
        ...
        int[] result = source.OrderBy(x => x).ToArray();
    ...
    public void OrderBy_정렬안정성_동일키순서유지()
    {
        var source = new[]
        {
            new { Id = 1, Key = "A" }, new { Id = 2, Key = "B" }, new { Id = 3, Key = "A" },
            new { Id = 4, Key = "B" }, new { Id = 5, Key = "A" }
        };
    ...
    {
        string[] source = new[] { "apple", "cherry", "banana" };
        string[] result = source.OrderByDescending(x => x).ToArray();
    ...
    {
        int[] source = new[] { 5, 2, 8, 1, 9, 3 };
        int[] result = source.OrderByDescending(x => x).ToArray();
    ...
    public void ThenBy_복합정렬_1차2차오름차순()
    {
        var source = new[]
        {
            new { Name = "Kim", Age = 30 }, new { Name = "Lee", Age = 25 }, new { Name = "Kim", Age = 20 },
            new { Name = "Lee", Age = 35 }
        };
    ...
    public void ThenByDescending_복합정렬_1차오름차순2차내림차순()
    {
        var source = new[]
        {
            new { Category = "A", Price = 100 }, new { Category = "B", Price = 200 },
            new { Category = "A", Price = 150 }, new { Category = "B", Price = 50 }
        };
