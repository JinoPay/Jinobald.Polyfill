...
public class EnumerableGroupingTests
{
    ...
    {
        string[] source = new[] { "apple", "banana", "apricot" };
        IGrouping<char, int>[] groups = source.GroupBy(x => x[0], x => x.Length).ToArray();
    ...
    {
        string[] source = new[] { "apple", "banana", "apricot", "blueberry", "avocado" };
        IGrouping<char, string>[] groups = source.GroupBy(x => x[0]).ToArray();
    ...
    {
        string[] source = new[] { "apple", "banana" };
        ILookup<char, string> lookup = source.ToLookup(x => x[0]);
    ...
    {
        string[] source = new[] { "apple", "banana", "apricot", "blueberry" };
        ILookup<char, string> lookup = source.ToLookup(x => x[0]);
