...
public static partial class ThreadingHelper
{
    ...
    public static bool SpinWaitRelativeExchangeUnlessNegative(ref int check, int value, out int lastValue)
    {
        ...
        {
            lastValue = Volatile.Read(ref check);
            if (lastValue < 0 || lastValue < -value) return false;
    ...
    {
        if (milliseconds < -1) throw new ArgumentOutOfRangeException(nameof(milliseconds));

        if (milliseconds == -1) return SpinWaitRelativeSet(ref check, value);
        ...
        while (true)
        {
            ...
            if (Milliseconds(TicksNow() - start) >= milliseconds) return false;
    ...
    public static bool SpinWaitSet(ref int check, int value, int comparand, int milliseconds)
    {
        ...
        while (true)
        {
            if (Interlocked.CompareExchange(ref check, value, comparand) == comparand) return true;
    ...
    public static bool SpinWaitUntil(Func<bool> verification, TimeSpan timeout)
    {
        ...
        if (milliseconds < -1L || milliseconds > int.MaxValue) throw new ArgumentOutOfRangeException(nameof(timeout));
        ...
        while (true)
        {
            if (verification.Invoke()) return true;
    ...
    {
        var spinWait = new SpinWait();
        while (Interlocked.CompareExchange(ref check, value, comparand) != comparand) spinWait.SpinOnce();
    }
    ...
    public static void SpinWaitUntil(Func<bool> verification)
    {
        ...
        var spinWait = new SpinWait();
        while (!verification.Invoke()) spinWait.SpinOnce();
    }

    public static void SpinWaitUntil(ref int check, int comparand)
    {
        var spinWait = new SpinWait();
        while (Volatile.Read(ref check) != comparand) spinWait.SpinOnce();
    }
    ...
    {
        var spinWait = new SpinWait();
        while (Volatile.Read(ref check) == null) spinWait.SpinOnce();
    }
}