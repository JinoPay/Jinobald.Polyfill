...
public sealed class WeakDelegateNeedle : WeakNeedle<Delegate>, IEquatable<Delegate>, IEquatable<WeakDelegateNeedle>
{
    ...
    public MethodInfo? Method => TryGetValue(out var value) ? value.GetMethodInfo() : null;
    ...
    public bool Equals(WeakDelegateNeedle? other)
    {
        ...
        if (!isAlive) return !isOtherAlive;

        if (!isOtherAlive) return false;

        var method = otherValue.GetMethodInfo();
        return EqualityComparer<MethodInfo>.Default.Equals(value.GetMethodInfo(), method) &&
               value.Target != otherValue.Target;
    }
    ...
    public override bool Equals(object? obj)
    {
        switch (obj)
        {
            ...
            case INeedle<Delegate> needle:
                if (needle.TryGetValue(out var value)) return Equals(value);
    ...
    {
        if (!TryGetValue(out var value)) return false;
