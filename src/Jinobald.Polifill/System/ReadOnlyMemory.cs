...
public readonly struct ReadOnlyMemory<T> : IEquatable<ReadOnlyMemory<T>>
{
    ...
    /// <summary>
    ///     For <see cref="ReadOnlyMemory{Char}" />, returns a new instance of string that represents the characters pointed to
    ///     by the memory.
    ...
    public override string ToString()
    {
        ...
        return string.Format("Memory<{0}>[{1}]", typeof(T).Name, Length);
    }
    ...
    public ReadOnlyMemory<T> Slice(int start)
    {
        ...
        return new ReadOnlyMemory<T>(_array, _index + start, Length - start);
    }
