...
public class AggregateException : Exception
{
    ...
    public override Exception GetBaseException()
    {
        ...
        {
            Exception item;
            if (result.InnerExceptions.Count != 1 || (item = result.InnerExceptions[0]) == null) return result;

            if (item is not AggregateException tmp) return item;
    ...
    public void Handle(Func<Exception, bool> predicate)
    {
        ...
        foreach (var exception in InnerExceptions)
            try
            {
                if (!predicate(exception)) failed.Add(exception);
            }
            catch
            {
        ...
        if (failed.Count > 0) throw new AggregateException(failed);
    }
