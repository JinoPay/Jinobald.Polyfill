...
public readonly struct Memory<T> : IEquatable<Memory<T>>
{
    ...
    ///     Thrown when <paramref name="array" /> is covariant and array's type is not
    ///     exactly T[].
    /// </exception>
    ...
    {
        return new ReadOnlyMemory<T>(memory._array, memory._index, memory.Length);
    }
    ...
    /// <summary>
    ///     For <see cref="Memory{Char}" />, returns a new instance of string that represents the characters pointed to by the
    ///     memory.
    ///     Otherwise, returns a <see cref="string" /> with the name of the type and the number of elements.
    /// </summary>
    public override string ToString()
    {
        if (typeof(T) == typeof(char)) return _array is string str ? str.Substring(_index, Length) : Span.ToString();
        ...
        return $"System.Memory<{typeof(T).Name}>[{Length}]";
    }
    ...
    public Memory<T> Slice(int start)
    {
        ...
        return new Memory<T>(_array, _index + start, Length - start);
    }
    ...
    ///     Returns true if the object is Memory or ReadOnlyMemory and if both objects point to the same array and have the
    ///     same length.
    /// </summary>
    ...
    public override int GetHashCode()
    {
        ...
        return _array != null ? HashCode.Combine(RuntimeHelpers.GetHashCode(_array), _index, Length) : 0;
    }
}
