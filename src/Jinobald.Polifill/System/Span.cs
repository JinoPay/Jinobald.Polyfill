...
public ref struct Span<T>
{
    ...
    public static Span<T> Empty => new(ArrayEx.Empty<T>());
    ...
    {
        var length = Length;
        if (destination.Length < length) throw new AggregateException(nameof(destination));
        for (var i = 0; i < length; i++) destination[i] = _array[_start + i];
    }
    ...
    {
        for (var i = 0; i < _array.Length; i++) _array[_start + i] = value;
    }
    ...
    {
        if (start > _length) throw new ArgumentOutOfRangeException(nameof(start));
    ...
    public T[] ToArray()
    {
        if (_length == 0) return ArrayEx.Empty<T>();
