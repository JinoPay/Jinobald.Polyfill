...
public sealed class FixedSizeQueue<T> : IProducerConsumerCollection<T>
{
    ...
    public bool TryAdd(T item)
    {
        ...
        if (preCount <= Capacity)
        {
            ...
            if (_entries.InsertInternal(index, item)) return true;
        }
    ...
    public bool TryTake(out T item)
    {
        if (_entries.Count > 0)
        {
            ...
            if (preCount >= 0)
            {
                ...
                if (_entries.RemoveAtInternal(index, out item)) return true;
            }
    ...
    public T Peek()
    {
        ...
        if (index < Capacity && index > 0 && _entries.TryGet(index, out var item)) return item;
