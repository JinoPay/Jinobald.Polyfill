...
public static class BucketHelper
{
    ...
    /// </remarks>
    public static bool InsertOrUpdateChecked<T>(this IBucket<T> bucket, int index, Func<T> itemFactory,
        Func<T, T> itemUpdateFactory, Predicate<T> check, out bool isNew)
    {
        if (bucket == null) throw new ArgumentNullException(nameof(bucket));

        if (itemFactory == null) throw new ArgumentNullException(nameof(itemFactory));

        if (itemUpdateFactory == null) throw new ArgumentNullException(nameof(itemUpdateFactory));
        ...
        while (true)
            if (isNew)
            {
                ...
                if (bucket.Insert(index, created, out _)) return true;

                isNew = false;
            }
            else
            {
                if (bucket.Update(index, itemUpdateFactory, check, out isNew)) return true;

                if (!isNew) return false; // returns false only when check returns false
            }
    }
    ...
    ///     The operation will be attempted as long as check returns true - this operation may starve.
    /// </remarks>
    public static bool InsertOrUpdateChecked<T>(this IBucket<T> bucket, int index, T item, Func<T, T> itemUpdateFactory,
        Predicate<T> check, out bool isNew)
    {
        if (bucket == null) throw new ArgumentNullException(nameof(bucket));
        ...
        while (true)
            if (isNew)
            {
                if (bucket.Insert(index, item, out _)) return true;
        ...
        out bool isNew)
    {
        ...
        while (true)
            ...
            {
                if (bucket.Update(index, _ => item, check, out isNew)) return true;
    ...
    public static bool TryGetOrInsert<T>(this IBucket<T> bucket, int index, Func<T> itemFactory, out T stored)
    {
        ...
        if (bucket.TryGet(index, out stored)) return false;

        if (itemFactory == null) throw new ArgumentNullException(nameof(itemFactory));

        var created = itemFactory.Invoke();
        if (!bucket.Insert(index, created, out stored)) return false;
    ...
    ///     The operation will be attempted as long as check returns true - this operation may starve.
    /// </remarks>
    public static void InsertOrUpdate<T>(this IBucket<T> bucket, int index, Func<T> itemFactory,
        Func<T, T> itemUpdateFactory, out bool isNew)
    {
        if (bucket == null) throw new ArgumentNullException(nameof(bucket));
        ...
        while (true)
            if (isNew)
            {
                ...
                if (bucket.Insert(index, created, out _)) return;

                isNew = false;
            }
            else
            {
                if (bucket.Update(index, itemUpdateFactory, Tautology, out isNew)) return;
            }
    }
