...
internal static class ArrayReservoir
{
    ...
    public static Pool<Array>? GetPool<T>(int index)
    {
        ...
        if (pools.TryGetValue(typeof(T), out var poolArray)) return poolArray?[index];
...
internal static class ArrayReservoir<T>
{
    internal static void DonateArray(T[] donation)
    {
        ...
        if (capacity == 0 || capacity < ArrayReservoir.MinCapacity || capacity > ArrayReservoir.MaxCapacity) return;
    ...
    {
        if (capacity == 0) return ArrayEx.Empty<T>();

        if (capacity < ArrayReservoir.MinCapacity) capacity = ArrayReservoir.MinCapacity;

        capacity = NumericHelper.PopulationCount(capacity) == 1 ? capacity : NumericHelper.NextPowerOf2(capacity);
        if (capacity > ArrayReservoir.MaxCapacity) return new T[capacity];

        var index = NumericHelper.Log2(capacity) - ArrayReservoir.MinCapacityLog2;
        var pool = ArrayReservoir.GetPool<T>(index);
        if (pool == null) return new T[capacity];

        if (pool.TryGet(out var result)) return (T[])result;
