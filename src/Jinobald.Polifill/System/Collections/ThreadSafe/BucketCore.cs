...
internal sealed class BucketCore : IEnumerable<object>, ISerializable
{
    ...
    private BucketCore(SerializationInfo info, StreamingContext context)
    {
        ...
        for (var subIndex = 0; subIndex < Math.Min(_capacity, contents.Length); subIndex++)
        {
    ...
    ~BucketCore()
    {
        ...
        if (GCMonitor.FinalizingForUnload) return;
    ...
    public IEnumerator<object> GetEnumerator()
    {
        ...
        if (arrayFirst == null || arraySecond == null || arrayUse == null) return Empty();
    ...
    public bool Do(int index, DoAction callback)
    {
        ...
        var arraySecond = Volatile.Read(ref _arraySecond);
        var arrayUse = Volatile.Read(ref _arrayUse);
        if (arrayFirst == null || arraySecond == null || arrayUse == null) return false;
    ...
    public bool DoMayIncrement(int index, DoAction callback)
    {
        ...
        if (arrayFirst == null || arraySecond == null || arrayUse == null || childFactory == null) return false;
    ...
    {
        if (indexFrom < 0) throw new ArgumentOutOfRangeException(nameof(indexFrom), "indexFrom < 0");

        if (indexTo < 0) throw new ArgumentOutOfRangeException(nameof(indexTo), "indexTo < 0");
    ...
    private static bool DoMayDecrement(ref int use, ref object? first, ref object? second, DoAction callback)
    {
        try
        {
            ...
            if (!callback(ref second)) return false;
        ...
        DoAction callback)
    {
        try
        {
            ...
            if (!callback(ref first)) return false;
    ...
    {
        if (Interlocked.Decrement(ref use) != 0) return;
    ...
    private IEnumerable<object> PrivateEnumerableRange(int indexFrom, int indexTo, int startSubIndex, int endSubIndex)
    {
        ...
        for (var subIndex = startSubIndex; subIndex < endSubIndex + 1; subIndex += step)
            try
            {
                ...
                {
                    if (foundFirst is not BucketCore core) continue;
