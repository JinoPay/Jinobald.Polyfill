...
public sealed class WeakDelegateCollection : WeakCollection<Delegate, WeakDelegateNeedle>
{
        ...
        : base(autoRemoveDeadItems)
    {
        ...
        else
        {
            ...
            _invokeAndClearWithException = (onException, input) =>
                guard.Execute(() => InvokeAndClearExtracted(onException, input));
        }
    }
    ...
    {
        foreach (var handler in ClearEnumerable()) handler?.DynamicInvoke(args);
    }
    ...
    {
        foreach (var handler in this) handler?.DynamicInvoke(args);
    }
}