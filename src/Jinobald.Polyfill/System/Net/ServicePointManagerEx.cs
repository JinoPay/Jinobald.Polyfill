...
public static class ServicePointManagerEx
{
    ...
    private static void ApplySecurityProtocol()
    {
        try
        {
            ...
            var servicePointManagerType = typeof(System.Net.ServicePoint).Assembly
                .GetType("System.Net.ServicePointManager");

            if (servicePointManagerType != null)
            {
                var securityProtocolProperty = servicePointManagerType.GetProperty(
                    "SecurityProtocol",
                ...
                if (securityProtocolProperty != null)
                {
                    ...
                    var frameworkSecurityProtocolType = servicePointManagerType.Assembly
                        .GetType("System.Net.SecurityProtocolType");

                    if (frameworkSecurityProtocolType != null)
                    {
                        var value = (int)_securityProtocol;
                        var enumValue = Enum.ToObject(frameworkSecurityProtocolType, value);
    ...
    private static void ApplyServerCertificateValidationCallback()
    {
        try
        {
            ...
            if (servicePointManagerType != null)
            {
                var callbackProperty = servicePointManagerType.GetProperty(
